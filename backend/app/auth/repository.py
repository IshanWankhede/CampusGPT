...
def count_recent_otps(db: Session, user_id: uuid.UUID, purpose: OtpPurpose) -> int:
...
def create_otp(
    db: Session, *, user_id: uuid.UUID, otp_hash: str, purpose: OtpPurpose, expires_at: datetime
) -> OtpVerification:
    record = OtpVerification(
        user_id=user_id, otp_hash=otp_hash, purpose=purpose, expires_at=expires_at
    )
    db.add(record)
    db.commit()
    db.refresh(record)
...
def get_latest_active_otp(
    db: Session, user_id: uuid.UUID, purpose: OtpPurpose
